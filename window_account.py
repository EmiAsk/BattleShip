...
class Account:
    ...
    def system_labels(self):
        ...
        self.games = self.font.render(f'All games: {data[3]}', True, [0, 0, 0])
        screen.blit(self.games, (100, 500))

        self.wins = self.font.render(f'Wins: {data[4]}', True, [0, 0, 0])
        screen.blit(self.wins, (100, 550))

        self.lose = self.font.render(f'Lose: {data[5]}', True, [0, 0, 0])
    ...
    def data_table(self, value=1):
        ...
        values = (value,)
        result = self.cur.execute('SELECT name, date_of_registration, player_level, all_games, wins, lose FROM \n'
                                  '        users WHERE id = ?', values).fetchall()
        self.con.close()
        for elem in result:
            return elem[0]
